...
function haveList(err, data) {
            ...
            if (index == fileListLen) {
                ...
                s3.getObject({Bucket: bucket, Key: currentFilename}, function (error, data) {
                    if (error) {
                        ...
                        var checkLog = currentFilename.search("_log");
                        //exclude consolidated.json from consolidation if it exists
                        if (currentFilename != '_consolidated.json' && currentFilename != '_config.json' && checkLog === -1) {
    ...
    function logAlerts() {
        ...
        //antivirus Alerts
        var antivirusString = "New Antivirus Alert! Antivirus logs: \n";
        if(alertAntivirus && antivirusAlertFound){
            antivirusString += antivirusAlertDetails;
            logOutput += antivirusString + " ";
            console.log(antivirusString);
        }else if(alertAntivirus == false){
            //do not include antivirus details in output
        }else if(!antivirusAlertFound){
            var noAlert = "No New Antivirus Alert. ";
            logOutput += noAlert;
            console.log(noAlert);
        }
